...
public class MenuDao {
	...
	public List<PizzaBaseVo> getPizzaOptions() {
		List<PizzaBaseMaster> pizzaBaseList = (List<PizzaBaseMaster>) redisTemplate.opsForValue().get(PIZZA_OPTIONS_KEY);
		if(pizzaBaseList == null) {
			pizzaBaseList = pizzaBaseMasterRepository.findAll(); 
			redisTemplate.opsForValue().set(PIZZA_OPTIONS_KEY, pizzaBaseList);
		}	
	...
	public List<PizzaToppingsVo> getPizzaToppings() {
		List<PizzaToppingsMaster> pizzaToppingsList = (List<PizzaToppingsMaster>) redisTemplate.opsForValue().get(PIZZA_TOPPINGS_KEY);
		if(pizzaToppingsList == null) {
			pizzaToppingsList = pizzaToppingsMasterRepository.findAll();
			redisTemplate.opsForValue().set(PIZZA_TOPPINGS_KEY, pizzaToppingsList);
		}	
